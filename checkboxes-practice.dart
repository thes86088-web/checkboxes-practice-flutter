...
class _ThisWidgetHasDataState extends State<ThisWidgetHasData> {
  ...
  Map<String, bool> attendanceToday = {
    "01": false,
    "02": false,
    "03": false,
    "04": false,
  ...
  void funcToToggleAttendanceValue(String requiredKey) {
    setState(() {
      attendanceToday[requiredKey] = !(attendanceToday[requiredKey]!);
    });
  }
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        Text("this page displays the attendance information"),
        ThisWidgetHasCheckBoxes(
          receivedAttendance: attendanceToday,
          copyOfReceivedAttendance: attendanceToday,
          funcToToggleAttendanceValue: funcToToggleAttendanceValue,
        ),
      ],
    );
  }
...
class ThisWidgetHasCheckBoxes extends StatelessWidget {
  ...
  ThisWidgetHasCheckBoxes({
    required this.receivedAttendance,
    required this.copyOfReceivedAttendance,
    required this.funcToToggleAttendanceValue,
  });
  ...
  Widget build(BuildContext context) {
    ...
    return ListView.builder(
      itemBuilder: (context, index) => AttendanceCard(
        rollNo: listOfKeys[index],
        attendanceValue: (receivedAttendance[listOfKeys[index]]!),
        funcToToggleAttendanceValue: funcToToggleAttendanceValue,
      ),
    );
  }
      ...
      child: ListTile(
        leading: Checkbox(
          ...
          onChanged: (bool? value) {
            funcToToggleAttendanceValue(rollNo);
          },
        ),
        title: Text(rollNo),
      ),
